...
char			*add_to_list(t_list **list, char *ret, int mode)
{
	char *tmp;
...
void			print_on_fd(int *pipe_fd, t_list **list)
{
	t_list		*cur;
	t_list		*tmp;
	char		*str;
...
static	void	read_heredoc(char *b, char *target, int *pipe_fd)
{
	...
	while (read(0, b, 4))
	{
		if (((ft_strcmp(target, "EOF") == 0 && CTRL_D)
			|| (ret && ft_strcmp(ret, target) == 0 && ENTER)))
		{
			free(ret);
		...
		if (BACK_SPACE)
			ret = heredoc_backspace_check(ret);
		else if (ENTER)
			ret = heredoc_enter_check(ret, &list);
		else
			ret = heredoc_show_check(b, ret);
		ft_bzero(b, 4);
	}
	reset_term();
}
